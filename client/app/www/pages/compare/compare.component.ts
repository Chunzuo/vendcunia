...
export class CompareComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.authService.isLoggedIn()  && this.authService.isTwoFA() == true) {
      this.socket.emit('getCompareList', this.authService.getUserId());
    }
  }

  public remove(compareId) {
      // const index: number = this.appService.Data.compareList.indexOf(product);
      // if (index !== -1) {
      //     this.appService.Data.compareList.splice(index, 1);
      // }
      this.socket.emit('removeCompare', {userId: this.authService.getUserId(), compareId: compareId});
  }

  public clear() {
    // this.appService.Data.compareList.length = 0;
    this.socket.emit('clearCompare', this.authService.getUserId());
  }
  ...
  private InitializeSocketListeners() {
    const thisObj = this;
    this.socket.on('getCompareListResponse', (compares) => {
      thisObj.compareList = compares;
      sessionStorage.setItem('compare_count', compares.length);
    });
  }
}
